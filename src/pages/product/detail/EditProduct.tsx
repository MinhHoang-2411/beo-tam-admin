...
const EditProduct = ({
  ...
  const [isOpenDateSaleOff, setIsOpenDateSaleOff] = useState<boolean>(
    !!productDetail.date_on_sale_from
  );
  const [checkedStockManagement, setCheckedStockManagement] = useState<boolean>(
    productDetail.detail.manage_stock
  );
  const [checkedLimited, setCheckedLimited] = useState<boolean>(
    productDetail.detail.sold_individually
  );
  ...
  const [selectedCategory, setSelectedCategory] = useState<any[]>(
    productDetail.categories.length
      ? productDetail.categories.map((cate) => cate.id)
      : []
  );
  ...
  const listImageDetailHadUrl = useAppSelector(
    (state) => state.product.listImageDetailHadUrl
  );
  //tab
  ...
  const {
    ...
    defaultValues: {
      ...
      date_on_sale_from: productDetail.date_on_sale_from,
      date_on_sale_to: productDetail.date_on_sale_to,
      sku: productDetail.sku,
      manage_stock: productDetail.detail.manage_stock,
      stock_quantity: productDetail.detail.stock_quantity,
      backorders: productDetail.detail.backorders,
      stock_status: productDetail.detail.stock_status,
      sold_individually: productDetail.detail.sold_individually,
      weight: productDetail.detail.weight,
      dimensions: productDetail.detail.dimensions,
      upsell_ids: productDetail.detail.upsell_ids,
      cross_sell_ids: productDetail.detail.cross_sell_ids,
  ...
  const deleteImageHadUrl = (file: any) => {
    dispatch(productActions.deleteImageDetailHadUrll(file));
  };
  ...
  const onSubmit: SubmitHandler<FieldValues> = (payloadForm) => {
    const payload: any = {
      ...
      sku: payloadForm.sku,
      regular_price: payloadForm.regular_price,
      sale_price: payloadForm.sale_price,
      date_on_sale_from: payloadForm.date_on_sale_from,
      date_on_sale_to: payloadForm.date_on_sale_to,
      categories: selectedCategory.map((cate) => {
        return { id: cate };
      }),
      detail: {
        ...productDetail.detail,
        stock_status: payloadForm.stock_status,
        manage_stock: payloadForm.manage_stock,
        stock_quantity: Number(payloadForm.stock_quantity),
        backorders: payloadForm.backorders,
        sold_individually: payloadForm.sold_individually,
        weight: payloadForm.weight,
        dimensions: payloadForm.dimensions,
      },
    };
    ...
    dispatch(productActions.updateProduct(payloadRequest));
  };

  useEffect(() => {
    dispatch(productActions.getListCategory({}));
    dispatch(
      productActions.plusImageDetailHadUrl(
        productDetail.images.map((img) => ({ preview: img.src }))
      )
    );
    return () => {
      dispatch(productActions.resetListImageDetailHadUrl());
    };
  }, [dispatch, productDetail.images]);
    ...
    <Stack spacing={2} p={2}>
      ...
      >
        ...
        <Stack direction="row" gap={1} flexWrap="wrap">
          {listImageDetailHadUrl.length
            ? listImageDetailHadUrl.map((img: any, i: number) => (
                <ImageUpload
                  width="24%"
                  src={img.preview}
                  onDelete={() => {
                    deleteImageHadUrl(img);
                  }}
                />
              ))
            : null}
          {listImage.length
            ...
            : null}
          {(!!listImage.length || !!listImageDetailHadUrl.length) &&
          countTotalElements(listImage, listImageDetailHadUrl) < 8 ? (
            <DropzoneCustom
              Icon={AddCircleOutlineIcon}
              maxFile={
                8 - countTotalElements(listImage, listImageDetailHadUrl) > 1
                  ? 8 - countTotalElements(listImage, listImageDetailHadUrl)
                  : 1
              }
